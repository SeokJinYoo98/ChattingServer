...
YuJanggi는 C# TCP 서버와 Unity 클라이언트로 구성하는 온라인 장기 프로젝트입니다.
...
| `YuJanggi` | 보드, 기물 이동, 턴, 점수, 장군 및 승패 판정을 담당하는 장기 코어 |
...
- .NET SDK 10
- C#
- Unity 버전 및 API Compatibility Level은 Unity 클라이언트 작업을 시작하기 전에 확정

현재 .NET 프로젝트는 `net10.0`을 대상으로 합니다. Unity에서 공용 코드나 장기 코어를 직접 참조하려면 Unity가 지원하는 대상 프레임워크에 맞게 프로젝트 경계를 조정해야 합니다.
