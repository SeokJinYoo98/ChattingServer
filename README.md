...
자동 매칭, 이동 검증, 보드 동기화, 게임 채팅을 지원합니다.
...
클라이언트 요청 → 서버 메시지 처리 → GameSession → Core 검증 → 결과 전송

- **YuJanggiServer:** 접속, 메시지 처리, 매칭과 채팅 관리

- **ClientSession:** 플레이어와 연결 상태 관리

- **GameSession:** 게임별 Core 상태와 이동 요청 관리

- **YuJanggiCommon:** 서버와 클라이언트가 공유하는 메시지 계약

- **Core:** 턴, 기물 소유권과 합법 수 검증

온라인 대국의 최종 판정은 서버에서 수행합니다.
...
주요 메시지는 참가, 매칭, 게임 시작, 합법 수 조회, 이동, 채팅, 게임 종료, 오류로 구분됩니다.

이동 요청은 참가자, 현재 턴, 좌표, 기물 소유권을 확인한 뒤 Core에서 최종 검증합니다.

성공한 이동만 양쪽 플레이어에게 전체 보드 상태와 함께 전송합니다.
